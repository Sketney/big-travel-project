...
export default function Checklist({ tripId }: { tripId: string }) {
  ...
  async function toggle(it: Item) {
    ...
    setItems(prev => prev.map(x => x.id === it.id ? { ...x, is_completed: !it.is_completed } : x))
  }
        ...
        {items.map(it => (
          ...
          >
            ...
            <div className="flex-1">
              <div className={it.is_completed ? 'line-through text-gray-400' : 'font-medium'}>
                {it.title}
              </div>
              {it.due_at && (
                <div className="text-xs text-gray-500">К сроку: {it.due_at}</div>
              )}
            </div>
            <button onClick={() => remove(it)} className="text-red-700 hover:underline">
              Удалить
            </button>
          </li>
        ))}
        {items.length === 0 && (
          <li className="text-gray-500">Пока пусто — добавь первый пункт.</li>
        )}
      </ul>
    </div>
  )
