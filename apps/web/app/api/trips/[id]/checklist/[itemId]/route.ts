...
async function ownItem(sb: any, itemId: string, guestId: string | null) {
  const { data: item } = await sb.from('checklist_items').select('id, trip_id').eq('id', itemId).maybeSingle()
  if (!item) return null
  const { data: trip } = await sb.from('trips').select('id').eq('id', item.trip_id).eq('user_id', guestId ?? '__none__').maybeSingle()
